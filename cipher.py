...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        lower_len = len(string.ascii_lowercase)
        
        for index in range(lower_len):
            key = lower[index]
            value = lower[(index+shift) % lower_len]
            dict[key] = value

        for index in range(lower_len):
            key = upper[index]
            value = upper[(index+shift) % lower_len]
            dict[key] = value
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for currentShift in range(0, 26):
            ...
            decrypt_message = self.apply_shift(currentShift)
